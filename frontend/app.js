...
async function sendMultipart(url, files) {
...
async function fetchWorkspaceDetail(workspaceRel) {
  return fetchJSON(`/api/workspace/${workspaceRel}`);
}

async function fetchRunLog(workspaceRel, runId, offset) {
  return fetchJSON(`/api/workspace/${workspaceRel}/log?run_id=${encodeURIComponent(runId)}&offset=${offset}`);
}
...
function fileHref(workspaceRel, relPath) {
  return `/files/results/${workspaceRel}/${relPath}`;
}
...
async function uploadSourceDraft(detail) {
  ...
  await sendMultipart(`/api/workspace/${detail.workspace_rel}/drafts/import`, files);
  input.value = "";
  await render();
}
...
async function uploadTemplate(detail) {
  ...
  await sendMultipart(`/api/workspace/${detail.workspace_rel}/templates/import`, files);
  input.value = "";
  await render();
}
...
async function runMigration(detail, templateId = null) {
  ...
  const result = await sendJSON("POST", `/api/workspace/${detail.workspace_rel}/migrations`, {
...
async function saveConfig(detail) {
  const payload = {
    ...
    enforce_disclosure: document.getElementById("cfg-enforce-disclosure").checked,
  ...
  await sendJSON("PUT", `/api/workspace/${detail.workspace_rel}/config`, payload);
  await render();
}
...
async function recycleArtifact(detail, kind, itemId) {
  await sendJSON("POST", `/api/workspace/${detail.workspace_rel}/artifacts/${kind}/${itemId}/recycle`, {});
  await render();
}

async function restoreArtifact(detail, recycleId) {
  await sendJSON("POST", `/api/workspace/${detail.workspace_rel}/recycle/${recycleId}/restore`, {});
  await render();
}
    ...
    <div style="display:flex;flex-direction:column;gap:18px">
      ...
      <div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          ...
          <label style="display:flex;align-items:center;gap:8px;font-size:13px"><input type="checkbox" id="cfg-enforce-disclosure" ${detail.config?.enforce_disclosure ? "checked" : ""} /><span>Enforce Disclosure</span></label>
...
render().catch((e) => renderFatal(e?.message || "Initial render failed", e?.stack || ""));
setInterval(() => {
