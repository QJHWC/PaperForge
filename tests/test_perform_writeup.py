...
from engine.perform_writeup import ChktexResult, _sanitize_template_tex_file, compile_latex, generate_latex, perform_writeup
...
""".strip(),
    ...
    ok = generate_latex(Mock(), str(tmp_path), str(tmp_path / "paper.pdf"), enforce_disclosure=False, skip_chktex_fix=True)
...
def test_disclosure_disabled_does_not_modify_draft(tmp_path) -> None:
    ...
    _sanitize_template_tex_file(str(tex_path), enforce_disclosure=False)
