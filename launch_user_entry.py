...
from paperforge.provider import CredentialResolver
...
def _sanitize_cli_text(text: str) -> str:
    sanitized = redact_secrets(text).replace(str(ROOT), ".")
...
def _collect_effective_config(args: argparse.Namespace) -> dict[str, dict[str, str | None]]:
    ...
    uses_bailu = any(model == "bailu-turing" for model in model_candidates)
    openai_alias = "bailu_primary" if uses_bailu else "openai_primary"
    writeup_alias = "bailu_primary" if getattr(args, "writeup_model", None) == "bailu-turing" else "openai_writeup"
    return {
        "openai_api_key": {
            "value": resolver.resolve(openai_alias, legacy_names=("OPENAI_API_KEY",)),
            "source": "user-config-or-legacy-env",
        },
        ...
        "openai_writeup_api_key": {
            "value": resolver.resolve(
                writeup_alias,
                legacy_names=("OPENAI_WRITEUP_API_KEY", "OPENAI_API_KEY"),
            ),
            "source": "user-config-or-legacy-env",
        },
        ...
        "anthropic_api_key": {
            "value": resolver.resolve(
                "anthropic_primary",
                legacy_names=("ANTHROPIC_API_KEY",),
            ),
            "source": "user-config-or-legacy-env",
        },
...
def _validate_effective_config(args: argparse.Namespace, cfg: dict[str, dict[str, str | None]]) -> None:
    ...
    if args.claude_protocol == "openai":
        if not (cfg["openai_api_key"]["value"] or cfg["openai_writeup_api_key"]["value"]):
            raise SystemExit(
                ...
                "请在 PaperForge 用户配置目录的 credentials.json 中配置。"
            )
        return
...
def _apply_api_env(args: argparse.Namespace, cfg: dict[str, dict[str, str | None]]) -> dict:
    env = os.environ.copy()
...
def _display_value(value: str | None, *, empty_fallback: str = "(default)") -> str:
    if value is None:
...
def _print_effective_configuration(
    ...
    cfg: dict[str, dict[str, str | None]],
    cmd: list[str],
...
def _build_scientist_cmd(args: argparse.Namespace, passthrough: list[str]) -> list[str]:
    ...
    _append_opt(cmd, "--profile", "full")
    _append_opt(cmd, "--proposal-id", args.proposal_id)
...
def _build_mvp_cmd(args: argparse.Namespace, passthrough: list[str]) -> list[str]:
    ...
    _append_opt(cmd, "--profile", args.profile)
    _append_opt(cmd, "--proposal-id", args.proposal_id)
    ...
    _append_flag(cmd, "--run-experiment", args.run_experiment)
...
def _maybe_materialize_research_partner(
    *,
    ...
    args: argparse.Namespace,
    completed,
    materialize_env: dict[str, str] | None = None,
) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    scientist.add_argument("--proposal-id", required=True)
    ...
    mvp.add_argument("--run-experiment", action="store_true")
    mvp.add_argument(
        "--profile",
        choices=["writing-only", "research", "full"],
        default="writing-only",
    )
    mvp.add_argument("--proposal-id", default=None)
    ...
    mvp.set_defaults(skip_chktex_fix=None)
    ...
    writeup.set_defaults(skip_chktex_fix=None)
...
def main() -> None:
    ...
    for item in sys.argv[1:]:
        ...
        if option in _REMOVED_SECRET_OPTIONS:
            raise SystemExit(
                f"{option} 已移除；请将凭据保存到 PaperForge 用户配置目录。"
            )
    parser = build_parser()
