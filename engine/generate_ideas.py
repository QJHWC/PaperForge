...
def generate_ideas(
    ...
    idea_str_archive = []
    with open(osp.join(base_dir, "seed_ideas.json")) as f:
        seed_ideas = json.load(f)
    for seed_idea in seed_ideas:
        idea_str_archive.append(json.dumps(seed_idea))

    with open(osp.join(base_dir, "experiment.py")) as f:
        code = f.read()

    with open(osp.join(base_dir, "prompt.json")) as f:
        prompt = json.load(f)
...
def generate_next_idea(
        ...
        client,
        model,
        prev_idea_archive=None,
        num_reflections=5,
        max_attempts=10,
):
    if prev_idea_archive is None:
        prev_idea_archive = []
...
def search_for_papers(query, result_limit=10, engine="semanticscholar") -> None | list[dict]:
    if not query:
        return None
    if engine == "semanticscholar":
        ...
        def extract_info_from_work(work: Work, max_abstract_length: int = 1000) -> dict[str, str]:
            ...
            venue = "Unknown"
            for _i, location in enumerate(work["locations"]):
