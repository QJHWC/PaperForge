...
""",
    ...
    if num_reflections > 1:
        for _j in range(num_reflections - 1):
...
"""
    for paper, review in zip(
        fewshot_papers[:num_fs_examples], fewshot_reviews[:num_fs_examples], strict=False
    ):
...
def perform_improvement(review, coder):
    improvement_prompt = f'''The following review has been created for your research paper:
"""
{json.dumps(review)}
"""
