...
def render_ieee_tii_output(content: MigrationContent) -> str:
    ...
    return (
        rf"""\documentclass[journal,twoside,web]{{ieeecolor}}
\usepackage{{generic}}
\usepackage{{cite}}
\usepackage{{amsmath}}
...
def render_cjc_output(content: MigrationContent) -> str:
    ...
    return (
        rf"""\documentclass[10.5pt,compsoc]{{CjC}}
\usepackage{{CJKutf8}}
\usepackage{{graphicx}}
\usepackage{{footmisc}}
...
\begin{{CJK*}}{{UTF8}}{{gbsn}}
\begin{{center}}
{{\zihao{{2}}\textbf{{{content.title}}}}}
\vskip 5mm
{{\zihao{{4}} {content.authors}}}
\end{{center}}
\vskip 4mm
\noindent\textbf{{摘\quad 要}}\quad {content.abstract}
\par\vskip 2mm
