...
from paperforge.provider import ProviderRegistry, create_chat_completion
...
def _is_openai_compatible_model(model: str) -> bool:
    return model in OPENAI_COMPATIBLE_MODELS
...
def normalize_claude_model_name(model: str) -> str:
    ...
    if os.getenv("PAPERFORGE_FORCE_CLAUDE_OPUS_TO_SONNET", "0").strip().lower() in {
        ...
        "yes",
        "on",
    } and normalized == "claude-opus-4-6":
...
def _openai_chat_create(client, **payload):
    request_model = str(payload.pop("model"))
...
def _resolve_claude_openai_client_kwargs():
    ...
    config = registry.resolve("claude-openai-compatible", stage="writeup")
...
def _resolve_openai_client_kwargs(model: str):
    ...
    """
    registry = ProviderRegistry()
    stage = "writeup" if model == "gpt-5.2-xhigh" else "default"
    config = registry.resolve(model, stage=stage)
    kwargs = registry.openai_client_kwargs(config)
    kwargs["default_headers"] = _default_openai_headers()
    return kwargs
...
def _should_failover_anthropic(exc: Exception) -> bool:
    ...
    if isinstance(
            exc,
            anthropic.RateLimitError | anthropic.APITimeoutError | anthropic.APIConnectionError | anthropic.InternalServerError | anthropic.AuthenticationError | anthropic.PermissionDeniedError,
    ):
        return True
...
def get_batch_responses_from_llm(
    ...
    if _is_openai_compatible_model(model):
        ...
        for _ in range(n_responses):
            response_text, response_history = get_response_from_llm(
                msg,
                client,
                model,
                system_message,
            ...
            new_msg_history.append(response_history)
    elif 'gpt' in model or model.startswith("grok-") or (
