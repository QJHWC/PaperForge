...
def load_baseline_results(workspace: str) -> dict:
    baseline_path = osp.join(workspace, "baseline", "final_info.json")
    ...
    metadata = data.get("_paperforge", {}) if isinstance(data, dict) else {}
    if (
        not isinstance(metadata, dict)
        or metadata.get("provenance_kind") not in {"measured", "imported"}
        or metadata.get("eligible_for_claims") is not True
    ):
        return {}
...
def initialize_notes(workspace: str, title: str, description: str, overwrite: bool = False) -> str:
    ...
    content = [
        ...
        f"Results: {baseline}" if baseline else "No measured or imported baseline has been verified.",
        "Description: Results are included only when provenance is measured/imported and claim-eligible.",
        "",
        "## Authoring policy",
...
def run_experiment_once(workspace: str, python_bin: str = "python", run_index: int | None = None) -> dict:
    ...
    for idx in list_run_indices(workspace):
        ...
        try:
            ...
            if (
                ...
                continue
            dataset_items = [
                item for item in data.items() if item[0] != "_paperforge"
            ]
            dataset_name, dataset_obj = next(iter(dataset_items))
...
def _copy_file_unique(src: Path, dst_dir: Path, preferred_name: str | None = None) -> str:
